"""Smoke experiment for paired Case 2 altbase."""
...
from src.dgps import PairedCase2AltbaseDGP
...
def build_config(args: argparse.Namespace) -> Case2AltbaseSmokeConfig:
    """Build the reusable smoke config from CLI args."""

    return Case2AltbaseSmokeConfig(
        seed=args.seed,
...
def run_case2_altbase_once(config: Case2AltbaseSmokeConfig):
    """Run one reusable Case 2 altbase fit and return dataset, fit, metrics."""
    ...
    metrics = {
        ...
        "sigma2_miae": sigma2_miae(config.sigma2, result.covariance.sigma2_hat_t),
        "sigma2_rmise": sigma2_rmise(config.sigma2, result.covariance.sigma2_hat_t),
