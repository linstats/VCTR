"""Reduced-scale MATLAB-style reproduction script for Case 2."""
...
from src.dgps import Case2BaselineDGP
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-prefix", type=str, default="reproduce_case2_matlab")
    parser.add_argument("--output-dir", type=str, default=str(default_output_dir))
    return parser.parse_args()


def run(args: argparse.Namespace) -> tuple[list[Case2MatlabRecord], list[dict[str, float | int | str]]]:
    raw: list[Case2MatlabRecord] = []
    for n in args.n_values:
        for noise_type in args.noise_types:
            for coef_type in args.coef_types:
                for rep in range(args.n_rep):
                    seed = args.seed_base + rep
                    dataset = Case2BaselineDGP(
                        n=n,
                        R=5,
                        S=64,
                        p0=2,
...
def summarize(records: Iterable[Case2MatlabRecord]) -> list[dict[str, float | int | str]]:
    grouped: dict[tuple[int, str, str], list[Case2MatlabRecord]] = {}
    for rec in records:
...
def maybe_write_csv(raw: list[Case2MatlabRecord], summary: list[dict[str, float | int | str]], output_dir: str, output_prefix: str) -> None:
