"""Repeated paired Case 2 altbase simulation driver."""
...
class PairedCase2AltbaseRecord:
    n_subject: int
    coef_type: str
    rep: int
    seed: int
...
def run_one(
    *,
    ...
    try:
        ...
        return PairedCase2AltbaseRecord(
            n_subject=n_subject,
            coef_type=coef_type,
            rep=rep,
            seed=seed,
...
def write_run_config(run_root: Path, args: argparse.Namespace, total_jobs: int) -> None:
    run_config = {
        "script": "src/experiments/paired_case2_altbase_repetition.py",
...
def initialize_raw_csv(run_root: Path) -> None:
    ...
    fieldnames = list(PairedCase2AltbaseRecord.__dataclass_fields__.keys())
    with raw_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()


def append_raw_record(run_root: Path, record: PairedCase2AltbaseRecord) -> None:
    raw_path = output_paths(run_root)["raw"]
    fieldnames = list(PairedCase2AltbaseRecord.__dataclass_fields__.keys())
...
def print_progress_line(
    *,
    record: PairedCase2AltbaseRecord,
    completed_jobs: int,
    total_jobs: int,
    global_start: float,
    n_rep: int,
...
def summarize(records: Iterable[PairedCase2AltbaseRecord]) -> list[dict[str, float | int | str]]:
    grouped: dict[tuple[int, str, float, str, str, int, str, str], list[PairedCase2AltbaseRecord]] = {}
    for rec in records:
        grouped.setdefault(
            (
                rec.n_subject,
...
def run(args: argparse.Namespace) -> tuple[list[PairedCase2AltbaseRecord], Path]:
