"""Repeated paired Case 1 simulation driver."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--n-subject-values", type=int, nargs="+", default=[2000, 5000])
    ...
    parser.add_argument(
        ...
        type=str,
        default="3,3",
        help="Comma-separated beta vector. Default matches Case I: 3,3.",
    )
    ...
    parser.add_argument("--signal-bandwidth", type=float, default=None)
    ...
    parser.add_argument("--variance-bandwidth", type=float, default=None)
...
def write_run_config(run_root: Path, args: argparse.Namespace, total_jobs: int) -> None:
    ...
    run_config = {
        "script": "src/experiments/paired_case1_repetition.py",
...
def initialize_raw_csv(run_root: Path) -> None:
    ...
    fieldnames = list(PairedCase1Record.__dataclass_fields__.keys())
    with raw_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()


def append_raw_record(run_root: Path, record: PairedCase1Record) -> None:
...
def write_progress_snapshot(
    ...
    *,
    completed_jobs: int,
    total_jobs: int,
    records: list[PairedCase1Record],
    global_start: float,
) -> None:
...
def run(args: argparse.Namespace) -> tuple[list[PairedCase1Record], Path]:
...
def summarize(records: Iterable[PairedCase1Record]) -> list[dict[str, float | int | str]]:
    grouped: dict[tuple[int, str, float, str, str], list[PairedCase1Record]] = {}
    for rec in records:
        grouped.setdefault(
...
def print_summary(summary: list[dict[str, float | int | str]]) -> None:
    ...
    print("\nPaired Case 1 repetition summary")
    for row in summary:
        print(
