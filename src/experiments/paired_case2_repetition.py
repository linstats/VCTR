"""Repeated paired Case 2 simulation driver."""
...
from src.dgps import PairedCase2DGP
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--n-subject-values", type=int, nargs="+", default=[1000, 1500, 2000])
    ...
    parser.add_argument(
        ...
        type=str,
        default="3,3",
        help="Comma-separated beta vector. Default matches Case II: 3,3.",
    )
...
def maybe_warn_case2_stage1_sample_size(
    *,
    n_subject: int,
    R: int,
    S: int,
    ...
    if not flagged:
        return
    flagged_text = ", ".join(f"h={h:.4g} -> {obs:.1f}" for h, obs in flagged)
    print(
        "[warning] Case 2 stage-1 local regression appears under-sampled for these bandwidth candidates: "
        f"{flagged_text}; stage1_param_count={stage1_param_count}. "
        "Expect unstable A_hat, near-identical paired residuals, and unreliable Sigma_hat."
    )
...
def write_run_config(run_root: Path, args: argparse.Namespace, total_jobs: int) -> None:
    ...
    run_config = {
        "script": "src/experiments/paired_case2_repetition.py",
...
def initialize_raw_csv(run_root: Path) -> None:
    ...
    fieldnames = list(PairedCase2Record.__dataclass_fields__.keys())
    with raw_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()


def append_raw_record(run_root: Path, record: PairedCase2Record) -> None:
...
def print_summary(summary: list[dict[str, float | int | str]]) -> None:
    ...
    print("\nPaired Case 2 repetition summary")
    for row in summary:
        print(
