% code to reproduce experiment result in supplementary Case IV
...
n_dict = {400,800,1200,1600};    % sample size
n_n = length(n_dict);
% noise type 
...
pentype = 'SCAD'; 
if n == 400 && i_err == 1
    penparam_beta=0.08; penparam_gamma_c=0.06; penparam_gamma_v=0.10;
elseif n == 400 && i_err == 2
    penparam_beta=0.08; penparam_gamma_c=0.04; penparam_gamma_v=0.16;
elseif n ~= 400 && i_err == 1
    penparam_beta=0.04; penparam_gamma_c=0.04; penparam_gamma_v=0.07;
else
    penparam_beta=0.04; penparam_gamma_c=0.05; penparam_gamma_v=0.10;
end

% 3D varying coefficient
S1 = floor(S^(1/3)); S2 = floor(S^(1/3)); S3 = floor(S^(1/3)); 
sp_R = 3;
sp_S = 3;
bR = (1:R)'/R;
bS1 = (1:S1)'/S1;bS2 = (1:S2)'/S2;bS3 = (1:S3)'/S3;
...
for s2 = 1:sp_S
for s3 = 1:sp_S
s = S2*S1*(s3-1) + S1*(s2-1) + s1;
if s1+s2+s3 < 6
    At(:,r,s) = sin(2*pi*(t-0.5))*sqrt(bR(r))*sqrt(bS1(s1))*sqrt(bS2(s2))*sqrt(bS3(s3));
else 
    At(:,r,s) = sqrt(bR(r))*sqrt(bS1(s1))*sqrt(bS2(s2))*sqrt(bS3(s3));
end
end
end
end
