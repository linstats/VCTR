% this is the code of the prediction error of our VCTR model in case 3
% In this code, we compare the model performance under 5 difference
% situations. 
% 1. Our VCTR model with true model structure; 
% 2. Original local linear tensor regression model in Section 2.3, 
% 3-5. Refined local linear tensor regression model with 'Lasso',  'SCAD',
% and 'MCP' penalties.
...
% prediction error of original and refined kernel smoothing 
% for each rho = 0.1, 0.5, 0.9, 
% we draw its prediction error of original kernel smoothing, that with lasso penalty, that with SCAD penalty, that with MCP penalty. 

At_hat_gt = zeros(n,R,S,3);
err_hat_gt = zeros(n,3);

At_hat_sm = zeros(n,R,S,3);
err_hat_sm = zeros(n,3);

At_hat_lasso = zeros(n,R,S,3);
err_hat_lasso = zeros(n,3);

At_hat_scad = zeros(n,R,S,3);
err_hat_scad = zeros(n,3);

At_hat_mcp = zeros(n,R,S,3);
err_hat_mcp = zeros(n,3);
...
for i_fold = 1:k_fold
fprintf('This is %d-th fold, rho %.1f.\n', i_fold, rho);
...
y_hat_ref_test(use_flag_vary_test) = sum(X_test(use_flag_vary_test,:,:) .* At_hat_ref_test(use_flag_vary_test,:,:), [2,3]) + Z_test(use_flag_vary_test,:) * beta_hat_ref_test;
err_hat_ref_test = y_test - y_hat_ref_test;

At_hat_gt(test_flag,:,:, i_rho) = At_hat_ref_test;
err_hat_gt(test_flag, i_rho) = err_hat_ref_test;


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% original kernel smoothing
At_hat_sm_test = zeros(n_test, R, S);
use_flag_sm_test = true(n_test,1);
for i=1:n_test
ker = max(.75*(1-(t_train-t_test(i)).^2/(h1^2)),0);
ker = sqrt(ker/h1);

if sum(ker > 0) <= 1.1*(2*R*S+p0)
    use_flag_sm_test(i) = false;
    continue
end
...
At_hat_sm_test(i,:,:) = reshape(At_hat_i, R,S);
end
...
At_hat_sm_est = zeros(n_est, R, S);
use_flag_sm_est = true(n_est,1);
for i=1:n_est
ker = max(.75*(1-(t_train-t_est(i)).^2/(h1^2)),0);
ker = sqrt(ker/h1);

if sum(ker > 0) <= 1.1*(2*R*S+p0)
    use_flag_sm_est(i) = false;
    continue
end
...
para_hat = (XZ_star' * XZ_star) \ XZ_star' * y_star;
At_hat_i = para_hat(1:R*S);
At_hat_sm_est(i,:,:) = reshape(At_hat_i, R,S);
end

yte = y_est(use_flag_sm_est) - sum(X_est(use_flag_sm_est,:,:) .* At_hat_sm_est(use_flag_sm_est,:,:), [2,3]);
beta_hat_sm_test = (Z_est(use_flag_sm_est, :)'*Z_est(use_flag_sm_est, :) + eye(p0)*(1e-4))\(Z_est(use_flag_sm_est, :)'*yte);

y_hat_sm_test = y_test;
y_hat_sm_test(use_flag_sm_test) = sum(X_test(use_flag_sm_test,:,:) .* At_hat_sm_test(use_flag_sm_test,:,:), [2,3]) + Z_test(use_flag_sm_test,:) * beta_hat_sm_test;
err_hat_sm_test = y_test - y_hat_sm_test;

At_hat_sm(test_flag,:,:, i_rho) = At_hat_sm_test;
err_hat_sm(test_flag, i_rho) = err_hat_sm_test;
...
At_hat_lasso(test_flag,:,:, i_rho) = At_hat_ref_test;
err_hat_lasso(test_flag, i_rho) = err_hat_ref_test;
elseif strcmp(pentype, 'SCAD')
At_hat_scad(test_flag,:,:, i_rho) = At_hat_ref_test;
err_hat_scad(test_flag, i_rho) = err_hat_ref_test;
else
At_hat_mcp(test_flag,:,:, i_rho) = At_hat_ref_test;
err_hat_mcp(test_flag, i_rho) = err_hat_ref_test;
end
...
%{
mean(abs(err_hat_gt),1)
mean(abs(err_hat_sm),1)
mean(abs(err_hat_lasso),1)
mean(abs(err_hat_scad),1)
mean(abs(err_hat_mcp),1)
%}

save('case3_pred_err.mat', 'use_rows', 'At_hat_gt', 'err_hat_gt', 'At_hat_sm', 'err_hat_sm', 'At_hat_lasso', 'err_hat_lasso', 'At_hat_scad', 'err_hat_scad', 'At_hat_mcp', 'err_hat_mcp')
