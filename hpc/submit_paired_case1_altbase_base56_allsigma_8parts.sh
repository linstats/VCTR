...
for idx in "${!PART_REPS[@]}"; do
  ...
  echo "[submit] part=${part_id} n_rep=${n_rep} seed_base=${seed_base}"
  qsub_args=(
    qsub
    -v "PROJECT_ROOT=$PROJECT_ROOT,CONDA_MODULE=$CONDA_MODULE,CONDA_ENV_PATH=$CONDA_ENV_PATH,N_JOBS=$N_JOBS,RUN_ROOT=$RUN_ROOT,PART_ID=$part_id,N_REP=$n_rep,SEED_BASE=$seed_base"
    "$PBS_SCRIPT"
  )
  if [[ "$DRY_RUN" == "1" ]]; then
    printf '[dry-run]'
