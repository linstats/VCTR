...
bs=zeros(m,p1,p2);
% for j=1:m
% bs(j,1:(p1-j+1),1:(p2-j+1))=b(j:p1,j:p2);
% end
p1=64;
p2=p1;
b0=1:p1;
...
y=zeros(n,m);
for j=1:m
mu = X*b0 + double(ttt(tensor(reshape(bs(j,:,:),[p1 p2])), M, 1:2));
sigma = 1;  % noise level
y(:,j) = mu + sigma*randn(n,1);
end
...
t=s;
h1=.2;
Xstar=kron(X,[1 (s-t(2))/h1]);

Xstar=zeros(2,p1,p2,n*m);

sst=repmat((s'-t(2))/h1,n,1);
sst=reshape(sst,n*m,1);
Xstar(2,:,:,:)=ttm(M,repmat(sst,1,n),3);
Xstar(1,:,:,:)=ttm(M,ones(m*n,n),3);

Xm=repmat(X,m,1);
ystar=y.*repmat((s'-t(2))/h1,n,1);
ystar=reshape(ystar,n*m,1);

Xm=
Xstar=
ystar=
%add kernel weight
[~,beta_rk1,glmstats1] = kruskal_reg(Xm,Xstar,ystar,1,'normal');
toc;
