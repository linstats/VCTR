% this is the code to reproduce experiment results in case IV.

% in this code, we estimate our model performance under the overfitting
% situation, specifically, where 2RS+p0 > n. And in this situation, the first
% kernel smoothing is invaild. And we can only do the penalized regression
% and refined regression. 
% This code is to repeat our case III.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% estimation under different n and penalty type.
for n = [2000,3500,5000]
for pentype = ["LASSO", "SCAD", "MCP"]
%{
if strcmp(pentype, 'LASSO') 
penparam_beta=0.04; penparam_gamma_c=0.01; penparam_gamma_v=0.005;
elseif strcmp(pentype, 'SCAD') 
penparam_beta=0.04; penparam_gamma_c=0.03; penparam_gamma_v=0.07;
...
%}
if strcmp(pentype, 'LASSO') && n == 5000
penparam_beta=0.04; penparam_gamma_c=0.003; penparam_gamma_v=0.001;
elseif strcmp(pentype, 'LASSO') && n == 3500
penparam_beta=0.04; penparam_gamma_c=0.003; penparam_gamma_v=0.001;
elseif strcmp(pentype, 'LASSO') && n == 2000
penparam_beta=0.04; penparam_gamma_c=0.002; penparam_gamma_v=0.0009;
elseif strcmp(pentype, 'SCAD')
penparam_beta=0.04; penparam_gamma_c=0.03; penparam_gamma_v=0.07;
elseif strcmp(pentype, 'MCP')
penparam_beta=0.04; penparam_gamma_c=0.03; penparam_gamma_v=0.07;
...
for i_rep = 1:n_rep
fprintf('This is %d-th repeat for %s pentype and n=%d \n', i_rep, pentype, n)
...
fprintf(['This is repeated experiment for case 3,\n' ...
    'n = %d, penalty type is %s, the penalty of parameters are %.4f,%.4f,%.4f,\n' ...
    ...
    n, pentype, penparam_beta, penparam_gamma_c, penparam_gamma_v,...
