...
At_cal_hat_krus = zeros(n, p1, p2, n_At);
...
X2 = randn(p2_s,R,S);
for s = 1:S
X1_s = randn(p1_s,R); [X1_s, ~] = qr(X1_s, "econ"); X1(:,:,s) = X1_s; 
X2_s = randn(p2_s,R); [X2_s, ~] = qr(X2_s, "econ"); X2(:,:,s) = X2_s;
end    

lambda = ones(R,1);

At_cal = zeros(n,p1,p2);
X_cal = zeros(n,p1,p2);
for s1 = 1:S1
for s2 = 1:S2
s = (s1-1)*S1+s2;
...
X1_s = X1(:,:,s);
X2_s = X2(:,:,s);
At_cal(:, (s1-1)*p1_s+1:s1*p1_s, (s2-1)*p2_s+1:s2*p2_s) = ktensor(lambda, At_s, X1_s, X2_s);
X_cal(:, (s1-1)*p1_s+1:s1*p1_s, (s2-1)*p2_s+1:s2*p2_s) = ktensor(lambda, X_s, X1_s, X2_s);
end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mu = sum(X_cal.*At_cal,[2,3]) + Z * beta;
...
At_hat_vcplt_train = zeros(n_train, R, S);
use_flag_train = true(n_train,1);
for i=1:n_train
ker = max(.75*(1-(t_train-t_train(i)).^2/(h1^2)),0);
ker = sqrt(ker/h1);

if sum(ker > 0) <= 1.4*(2*R*S+p0)
    use_flag_train(i) = false;
    continue
end
...
para_hat = (XZ_star' * XZ_star) \ XZ_star' * y_star;
At_hat_i = para_hat(1:R*S);
At_hat_vcplt_train(i,:,:) = reshape(At_hat_i, R,S);
end

yte = y_train(use_flag_train) - sum(X_train(use_flag_train,:,:) .* At_hat_vcplt_train(use_flag_train,:,:), [2,3]);
beta_hat_vcplt_test = (Z_train(use_flag_train, :)'*Z_train(use_flag_train, :) + eye(p0)*(1e-4))\(Z_train(use_flag_train, :)'*yte);
...
X_krus_train = tensor(permute(X_cal_train, [2,3,1]));
...
At_cal_hat_krus_test = reshape(At_cal_hat_krus_test, [1,p1,p2]);
At_cal_hat_krus_test = repmat(At_cal_hat_krus_test, [n_test,1,1]);

At_cal_hat_krus(test_flag,:,:,i_At) = At_cal_hat_krus_test;

y_hat_krus_test = sum(X_cal_test .* At_cal_hat_krus_test, [2,3]) + Z_test * beta_hat_krus_test;
...
X1_s = X1(:,:,s);
X2_s = X2(:,:,s);
aux_mat = diag(lambda) * khatrirao(X2_s, X1_s)';
At_cal_hat_krus_test_s = At_cal_hat_krus_test(:, (s1-1)*p1_s+1:s1*p1_s, (s2-1)*p2_s+1:s2*p2_s);
...
save('case1_n2000.mat', 'use_flag','err_gt', 'At_hat_vcplt', 'err_hat_vcplt', 'At_hat_ptt', 'err_hat_ptt', 'At_hat_krus', 'err_hat_krus');



%{
mae_At_vcplt = mean(abs(At(use_flag,:,:) - At_hat_vcplt(use_flag,:,:)),'all');
rmse_At_vcplt = sqrt(mean((At(use_flag,:,:) - At_hat_vcplt(use_flag,:,:)).^2,'all'));
mae_err_vcplt = mean(abs(err_hat_vcplt(use_flag)),'all');
rmse_err_vcplt = sqrt(mean(err_hat_vcplt(use_flag,4).^2, 'all'));
...
mae_At_ptt = mean(abs(At(use_flag,:,:) - At_hat_ptt(use_flag,:,:)),'all');
rmse_At_ptt = sqrt(mean((At(use_flag,:,:) - At_hat_ptt(use_flag,:,:)).^2,'all'));
mae_err_ptt = mean(abs(err_hat_ptt(use_flag)),'all');
rmse_err_ptt = sqrt(mean(err_hat_ptt(use_flag).^2, 'all'));
...
mae_At_krus = mean(abs(At - At_hat_krus),'all');
rmse_At_krus = sqrt(mean((At - At_hat_krus).^2,'all'));
mae_At_cal_krus = mean(abs(At_cal - At_cal_hat_krus),'all');
rmse_At_cal_krus = sqrt(mean((At_cal - At_cal_hat_krus).^2,'all'));
mae_err_krus = mean(abs(err_hat_krus),'all');
rmse_err_krus = sqrt(mean(err_hat_krus.^2, 'all'));
